...
// ~20s, so the calling route sets maxDuration to cover it; the fetch is bounded
// a little under that. Returns one image URL's 768-d L2-normalized embedding.
...
export async function embedImageUrl(url: string): Promise<number[]> {
  ...
  if (!vec || vec.length !== EMBED_DIM) {
    throw new Error("embed worker returned no usable vector (image decode failed)");
  }
  return vec;
}
